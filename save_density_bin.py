...
def sample_grid(n: int) -> np.ndarray:
    """Grid of points over [0,1]³.

    Parameters
    ----------
    n : int
        Number of points.
        Must satisfy n=m³ for some integer m.

    Returns
    -------
    points : (N, 3) numpy.Tensor
        Points.

    Raises
    ------
    ValueError
        If `n` is not satisfy n=m³ for some integer m.

    """
    m = round(n ** (1.0 / 3.0))
    if m ** 3 != n:
        raise ValueError(f'n ({n}) does not satisfy n=m^3={m ** 3} for an integer m.')

    cell_size = 1.0 / m
    steps = np.arange(start=0.0, stop=m, step=1.0) + 0.5

    cell_centers_1d = steps * cell_size

    grid = np.meshgrid(*([cell_centers_1d] * 3), indexing='ij')
...
def shell_decay(f: np.ndarray, c: float, t: float, sigma: float) -> np.ndarray:
    ...
    return np.exp(-(excess * excess) / (2.0 * sigma * sigma + ZERO_TOL))
...
def build_parser() -> ArgumentParser:
    ...
    parser = ArgumentParser(
        description='Write density field to binary file for FEM homogenization.',
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help='FEM grid resolution.',
    )

    parser.add_argument(
        '--field',
        choices=DENSITY_FIELDS,
        required=True,
        help='Density field type.',
    )
...
def main() -> None:
    ...
    points = sample_grid(n=args.res ** 3)

    density_field = DENSITY_FIELDS[args.field]
    rho = density_field(points=points)
    rho = np.clip(a=rho, a_min=ZERO_TOL, a_max=1.0)
    ...
    with open(file=args.file, mode='wb') as f:
